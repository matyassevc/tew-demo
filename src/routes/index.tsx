  ...
  return (
    <>
      <section className="mx-auto max-w-7xl px-5 pb-24 pt-14 sm:px-8 sm:pt-20 lg:px-12 lg:pb-36">
        <div className="grid items-end gap-10 lg:grid-cols-12">
          <Reveal className="lg:col-span-8">
            <h1 className="text-5xl font-extrabold leading-[0.94] text-foreground sm:text-7xl lg:text-8xl xl:text-9xl">
              Výroba eurooken a dveří na míru
            </h1>
          </Reveal>
          <Reveal delay={0.15} className="pb-2 lg:col-span-4">
      ...
      <section className="mx-auto grid max-w-7xl gap-12 border-t border-border px-5 py-20 sm:px-8 sm:py-28 lg:grid-cols-12 lg:px-12 lg:py-36">
        <Reveal>
          <p className="text-7xl font-light text-border">01</p>
        </Reveal>
        <Reveal delay={0.1} className="lg:col-span-4">
          <h2 className="text-4xl font-extrabold leading-tight sm:text-5xl">Tradice, která formuje budoucnost</h2>
        </Reveal>
        <Reveal delay={0.2} className="space-y-6 text-base leading-8 text-muted-foreground lg:col-span-6">
      ...
      <section className="bg-footer text-footer-foreground">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-28 lg:px-12">
          <div className="grid gap-8 border-b border-footer-line pb-12 lg:grid-cols-2 lg:items-end">
            <Reveal><h2 className="text-4xl font-extrabold leading-tight sm:text-6xl">Od oken po vstupní portály</h2></Reveal>
            <Reveal delay={0.15}><p className="max-w-lg text-footer-muted lg:justify-self-end">Devět výrobkových řad, které přizpůsobíme rozměrem, materiálem i povrchovou úpravou konkrétní stavbě.</p></Reveal>
          </div>
          <div className="divide-y divide-footer-line">
            {products.slice(0, 5).map((product, index) => (
              <Reveal key={product.slug} delay={index * 0.06}>
              <Link to="/sortiment/$slug" params={{ slug: product.slug }} className="group grid gap-3 py-7 sm:grid-cols-[4rem_1fr_auto] sm:items-center">
          ...
          <Reveal delay={0.1}><Button asChild variant="outline" className="mt-10 border-footer-muted text-footer-foreground hover:bg-background hover:text-foreground"><Link to="/sortiment">Celý sortiment</Link></Button></Reveal>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-28 lg:px-12">
        <div className="grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4"><h2 className="text-4xl font-extrabold">Kvalita v každém detailu</h2></Reveal>
          ...
          {[
            ...
            <Reveal key={title} delay={index * 0.1} className="border-t border-border pt-7">
      ...
      <section className="border-y border-border bg-muted">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 sm:py-28 lg:grid-cols-2 lg:px-12">
          <Reveal><PhotoPlaceholder label="[ reference ]" className="aspect-[4/3]" /></Reveal>
          <Reveal delay={0.15} className="flex flex-col justify-center lg:pl-10"><h2 className="text-4xl font-extrabold leading-tight sm:text-5xl">Stovky zakázek po celé České republice</h2><p className="mt-6 max-w-lg leading-8 text-muted-foreground">Prohlédněte si ukázky eurooken, vchodových dveří a vrat v rodinných domech i dalších stavbách.</p><Button asChild variant="outline" className="mt-8 w-fit"><Link to="/reference">Prohlédnout reference</Link></Button></Reveal>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-32 lg:px-12">
        <div className="grid gap-8 lg:grid-cols-12 lg:items-end"><Reveal className="lg:col-span-8"><h2 className="text-5xl font-extrabold leading-[1.02] sm:text-7xl">Řekněte nám, co potřebujete vyrobit.</h2></Reveal><Reveal delay={0.15} className="lg:col-span-4"><p className="mb-7 leading-7 text-muted-foreground">Ozveme se Vám do 2 pracovních dnů a probereme další postup.</p><Button asChild size="lg"><Link to="/poptavka">Nezávazná poptávka <ArrowRight /></Link></Button></Reveal></div>
      </section>
    </>
  );
}
